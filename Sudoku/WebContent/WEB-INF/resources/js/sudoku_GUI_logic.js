...
{
	var $domobj = $("#sudoku");
	$domobj.addClass("col-lg-8 col-lg-offset-4 col-md-8 col-md-offset-4 col-xs-10 col-xs-offset-2");
  ...
  for (var y = 0; y < data.length; y++)
  {
    ...
    for (var x = 0; x < data[y].length; x++)
    {
      ...
      data[y][x] = new Cell(new Vec2d(x, y), numberCell, this); //create vertical column
		}
	...
	this.importPuzzle = function(puzzleStr)
  {
    ...
    for (var i = 0; i < puzzleStr.length; i++)
    {
	  ...
	  this.moveSelected = function(xOffset, yOffset)
	  {
	    ...
	    while (newX < 0) newX += 9; //go in the opposite cell (if i'm at the border left cell i go in the right border cell)
	    var newY = (selectedCell.y + yOffset) % 9;
	    while (newY < 0) newY += 9; //go in the opposite cell (same reasoning for the column)
	  ...
	  this.setSelectedValue = function(val)
	  {
	    ...
	    {
	      deselectNum(selectedNum); //deselect all the number that are not equal to the number inserted (in the cell selected everything is highlighted blue, while for the other equal number in the matric the border are highlighted blue)
	      selectedNum = val;
...
function Cell(pos, current_cell, parent, locked, value)
{
  ...
  {
  	$current_cell.removeClass("secondarySelected sameNumber").addClass("selectedCell");
  };
